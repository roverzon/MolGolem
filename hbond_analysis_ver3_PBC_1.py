...
#	data = pbc_organized_data(pbc_data)
#	count_hydrogen_bond(data)
...
if __name__ == "__main__":
	...
	record_file_name = "mpa_8_co2_4_pbc_hb.txt"
	fp = open(file_name)
	fp_r = open(record_file_name,"w")
	text_stack = []
	fp_r.write("i , hydrogen bond \n")
	while True:
		...
		#	print text_stack[0][1] 
			breaking_line =  int(text_stack[0][0]) + 2 
				...
				print total 
                        	text_stack[:] = []
                        	fp_r.write( str( count / breaking_line ) +  " , " + str(total)  + "\n" )
