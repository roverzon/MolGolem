...
#	print center_of_mass 
	...
	for i in  range(len(center_of_mass)):
		for  j in range(i,len(center_of_mass)):
			 del_x = center_of_mass[i][0] - center_of_mass[j][0] 
                         del_y = center_of_mass[i][1] - center_of_mass[j][1]
                         del_z = center_of_mass[i][2] - center_of_mass[j][2]
...
def operation(data):
	...
	avg_dis = count_dis(center_of_mass)	
	return  avg_dis 
...
if __name__ == "__main__":
	...
	record_file_name = "mpa_mpa_average_distance.txt"
	fp = open(file_name)
	fp_r = open(record_file_name,"w")
	text_stack = []
	fp_r.write("i ,  MPA-MPA Distance \n")
	while True:
		...
		if not file_text: 
			...
			breaking_line =  int(text_stack[0][0]) + 2 
				...
				avg_dis = operation(data)
                        	text_stack[:] = []
        #                	fp_r.write( str( count / breaking_line ) +  " , " + str(avg_dis)  + "\n" ) 
				break 
